...
- **Yellow 관련 메시지만 답장**: 그룹에서는 Yellow 관련 내용·봇 멘션·봇 답글에만
  반응 (모든 메시지에 답하지 않아 채팅방이 깔끔)
- **리더보드 & 데일리 리포트**: 멤버별 활동량, 화제 키워드, 유저 질문, 스팸 차단 수를
  집계해 매일 운영자에게 개인 DM으로 요약 보고 (Claude가 다음에 올릴 정보 메시지도 제안)
- **간간히 정보 제공**: 자동 정보 포스팅은 하루 2~3회로 제한하고, 사람들이 실제로
  대화 중일 때만 발송 (봇이 혼자 떠드는 상황 방지)
- **트위터 스크래핑**: `@yellow` + `@Yellow__Korea` 트윗 → 채팅방/구독자 자동 전달
- **AI 챗봇**: Claude AI로 Yellow 정보 제공
